...
.factory('cacheHttpInterceptor', ['$q', '$log', 'securityInfo','APP_CONFIG', 'securityRetryQueue', '$injector','CacheFactory','CacheUtil','AppUtil',
	function(q, log, info, appConfig, queue, $injector,CacheFactory,CacheUtil,AppUtil) {
		return {
...
.factory('myHttpInterceptor', ['$q', '$log', 'securityInfo','APP_CONFIG', 'securityRetryQueue', '$injector','CacheFactory','CacheUtil','AppUtil',
	function(q, log, info, appConfig, queue, $injector,CacheFactory,CacheUtil,AppUtil) {		
		return {
			request: function(config) {
				var cache;
				...
				if(config.intercetorParams && config.intercetorParams.api){
					var apiName = config.intercetorParams.api === true ?'':  config.intercetorParams.api;
					var host =  config.secure? appConfig['Secure'+apiName+'Host'] : appConfig[apiName+'Host'];
					config.url = config.secure ? host+config.url : host+config.url;
					delete config.intercetorParams.api;
				}
				// end handle interceptor params
				if(info.apiKey && info.apiKey.AccessToken){
					if(!config.noToken && config.url.indexOf('.html') < 0){
