...
.factory('CacheUtil',['APP_CONFIG', 'CacheFactory' , function(APP_CONFIG, CacheFactory){
	var util = {
		...
		getOfflineHttpCache: function(){
			var cache = CacheFactory.get('off_http');
			if(!cache){
				cache = CacheFactory('off_http', {
		...
		getGuestCache: function(){
			var cache = CacheFactory.get('guest');
			if(!cache){
				cache = CacheFactory('guest', {
					maxAge: 1000*60*60*24*7, // Items added to this cache expire after 1 weeks.
					storageMode: 'localStorage',
					capacity: 200,
					deleteOnExpire: 'aggressive'
